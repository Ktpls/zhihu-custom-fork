...
(() => {
  ...
  async function removeBlockUserOnZhihuServer(urlToken) {
    const headers = store.getFetchHeaders();
  ...
  async function blockUserOnZhihuServer(urlToken) {
    const headers = store.getFetchHeaders();
    await fetch(`https://www.zhihu.com/api/v4/members/${urlToken}/actions/block`, {
      method: "POST",
      headers: new Headers({
  ...
  var onPushBlacklistToZhihu = async () => {
    return;
  ...
  var onTransferZhihuBlacklistToLocal = async () => {
  ...
  var MAX_BLOCKED_USERS_DISPLAY_COUNT = 100;
  var renderBlockedUserList = (domMain, list, listType) => {
    ...
    const moreTip = list.length > MAX_BLOCKED_USERS_DISPLAY_COUNT ? `<div class="ctz-black-list-limit-tip" style="padding:4px 8px;font-size:12px;color:#999;">仅显示前 ${MAX_BLOCKED_USERS_DISPLAY_COUNT} 条记录（共 ${list.length} 条）</div>` : "";
  ...
  var processingData = async (nodes) => {
    ...
    for (let i = 0, len = nodes.length; i < len; i++) {
      ...
      if (!message2 && blockAnswerShorterThanThreshOnAnswer > 0) {
        const richContent = nodeItem.querySelector(".RichContent-inner");
        if (richContent) {
          const contentText = richContent.innerText || "";
          if (contentText.length < blockAnswerShorterThanThreshOnAnswer) {
